...
public class ShortMapUtil {
    ...
    public ShortMap bitmapToByteMap(Bitmap bitmap) {
    ...
    private ShortMap outputByteMap(int pixels[], int width, int height) {
...
//        }
    ...
    public Bitmap byteMapToBitmap(ShortMap ShortMap){
        Bitmap bitmap = Bitmap.createBitmap(ShortMap.width, ShortMap.height, Bitmap.Config.ARGB_8888);
        int index = 0;
        for(int y = 0; y < bitmap.getHeight(); y++){
            for(int x = 0; x < bitmap.getWidth(); x++) {
                int pixel = colorList.get(ShortMap.byteMap[index++] & 0xFFFF).color | 0xFF000000;
